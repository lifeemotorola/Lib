...
| `tests/` | Playwright UI regressions (`ui.py`), all-subject regression (`regress.py`), pure sequence unit test (`book.js`), and `notes-verbatim.js` (Node check that every `study[]` block list renders as-is). |
...
- A unit may also carry `study[]` — a block list (`h3`/`p`/`bul`/`num`/`rule`)
  transcribed **as-is from the official course text**. When present, the
  Study Notes page renders it verbatim instead of the auto-assembled page.
  Any `p`/`bul`/`num`/`instr` text supports inline `**bold**` markup
  (converted to `<b>`), used for key terms throughout the course text.

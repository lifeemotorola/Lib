/* General Science exercise generation engine.
   Emits the SAME block model as the English and French engines, so the shared
   renderer and .docx exporter are reused. Registered on window.GEN_SC. */
(function (root) {
  "use strict";
  ...
  function wsTerms(t) {
    return {
      blocks: [
        { k: "h3", t: "A · Key Terms — Science Vocabulary" },
        { k: "instr", t: "Read each term after your teacher. Copy the term, its meaning and the example into your science journal." },
  ...
  /* H · Label the diagram / parts and functions */
  function wsDiagram(t) {
    var d = t.diagram;
    if (!d || !d.parts || !d.parts.length) return null;
    return {
      blocks: [
        { k: "h3", t: "H · Label and Explain — " + d.title },
        { k: "instr", t: d.caption + " Then draw the diagram neatly in your science journal and label it." },
        { k: "table", head: ["Part / stage", "What it is or what it does"],
          rows: d.parts.map(function (p) { return [p.p, ""]; }) },
        { k: "p", t: "Drawing space — draw and label the diagram below:" },
        { k: "lines", n: 8 }
      ],
      key: d.parts.map(function (p, i) { return (i + 1) + ". " + p.p + " — " + p.f; })
    };
  }

  /* I · Experiment / investigation write-up */
  function wsExperiment(t) {
    var e = t.experiment;
    if (!e) return null;
    return {
      blocks: [
        { k: "h3", t: "I · Investigation — " + e.title },
        { k: "instr", t: "Carry out the activity in your group. Follow all safety rules and never taste any substance. Then complete the write-up." },
        { k: "p", t: "AIM: " + e.aim },
        { k: "p", t: "MATERIALS NEEDED:" },
        { k: "bul", items: e.materials },
        { k: "p", t: "METHOD — follow these steps:" },
        { k: "num", items: e.steps },
        { k: "space" },
        { k: "p", t: "MY PREDICTION (write before you begin):" },
        { k: "lines", n: 2 },
        { k: "p", t: "WHAT I OBSERVED:" },
        { k: "lines", n: 4 },
        { k: "p", t: "MY CONCLUSION — what does this show?" },
        { k: "lines", n: 3 }
      ],
      key: [
        "Expected observation: " + e.expect,
        "Expected conclusion / explanation: " + e.why,
        "Award marks for: a clear prediction, accurate observation, and a conclusion that matches the results."
      ]
    };
  }
  ...
  /* K · Science journal / drawing task */
  function wsJournal(t) {
    return {
      blocks: [
        { k: "h3", t: "K · Science Journal" },
        ...
        { k: "lines", n: 3 },
        { k: "p", t: "3. One thing I observed for myself:" },
        { k: "lines", n: 2 },
        { k: "p", t: "4. Draw and label something from this unit:" },
        { k: "lines", n: 8 },
        { k: "p", t: "5. One question I still want to answer:" },
        { k: "lines", n: 2 }
      ],
      key: ["Open response. Award marks for correct use of key terms, accurate observation and a neat labelled drawing."]
    };
  }

  /* L · Spelling and dictation of key terms */
  function wsSpelling(t, n, r) {
    var sel = pick(t.terms, n, r);
    if (!sel.length) return null;
    return {
      blocks: [
        { k: "h3", t: "L · Spelling & Vocabulary Test — Key Terms" },
  ...
  var SHEETS = {
    ...
    classify:   { label: "Classification & sorting",   fn: wsClassify },
    diagram:    { label: "Label the diagram",          fn: function (t) { return wsDiagram(t); } },
  ...
  function semesterExam(topics, sem, r) {
    ...
    topics.forEach(function (t) {
      ...
      if (t.diagram) dpool.push(t.diagram);
    });
    var g = topics[0].grade, blocks = [], key = [], q = 0, each = 2, n = 10;

    blocks.push({ k: "h2", t: "SEMESTER " + sem.toUpperCase() + " EXAMINATION — GRADE " + g + " GENERAL SCIENCE" });
    blocks.push({ k: "table", head: ["Name", "Class", "Date", "Score"],
    ...
    sec("SECTION A — Science vocabulary", "Write the meaning of each term.",
    ...
    /* Section E — diagram and application */
    var dg = pick(dpool, 1, r)[0];
    blocks.push({ k: "h3", t: "SECTION E — Diagram and application   (10 marks)" });
    if (dg) {
      blocks.push({ k: "instr", t: "1. Draw and label: " + dg.title + ". Beside each label, state what it is or what it does. (6 marks)" });
      blocks.push({ k: "lines", n: 8 });
      key.push("Section E (1) — " + dg.title + ": " + dg.parts.map(function (p) { return p.p + " = " + p.f; }).join("; "));
    }
  ...
  function buildPack(opts) {
    ...
    doc.push({ k: "h1", t: "GENERAL SCIENCE — GRADE " + opts.grade });
    doc.push({ k: "h2", t: "Pupil Workbook & Assessment Pack" });
    doc.push({ k: "p", t: "Elementary General Science · Liberian National Curriculum", i: true });
    doc.push({ k: "space" });
  ...
  root.GEN_SC = { buildPack: buildPack, SHEETS: SHEETS };
})(window);
